...
*/
	$(function(){
		//缓存思想：将后台数据放置在页面的可以存放数据的空间内，页面使用快速
		...
		$(".oper").click(function(){
			...
			//ajax获取当前订单明细对应的剩余未入库数量
			//使用当前订单的uuid
			var odmUuid = $nowTr.attr("odm");
			$.post("orderDetail_ajaxGetSurplusByUuid.action",{"om.uuid":odmUuid},function(data){
				//删除所有已经存在的入库行
				$(".in").remove();
				
				//data中包含 data.surplus
		...
		$(".ajaxIn").live("click",function(){
			...
			$.post("order_ajaxInGoods.action",jsonParam,function(data){
				//data中包含 data.odm.surplus data.odm.num data.msg
				var msg = data.msg;
				if(msg.length > 0){
					alert(msg);
					return;
				...
				//如果所有商品明细都入库完毕，隐藏两个，显示两个
				//if(如果所有的明细都入库完毕){
				//if(如果未入库的明细项只有一个，并且这一次入库操作将它全部入库完毕了){
